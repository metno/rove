...
def send_spatial(stub):
    ...
    time = ts.Timestamp(seconds=int(datetime(2023, 6, 26, hour=14, tzinfo=timezone.utc).timestamp()))
    tests = ["dip_check", "step_check"]
    polygon = [rove.GeoPoint(lat=59.93, lon=10.05),rove.GeoPoint(lat=59.93, lon=11.0),rove.GeoPoint(lat=60.25, lon=10.77),]
    request = rove.ValidateSpatialRequest(spatial_id=spatial_id, time=time, tests=tests, polygon=polygon)
